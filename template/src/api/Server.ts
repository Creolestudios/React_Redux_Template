...
import {localStorageHelper, StorageKeys} from '../Utils/LocalStorageHelper';
...
userAuthFetch.interceptors.request.use(config =>
  localStorageHelper
    ...
    .then(tokenResponse => {
      config.headers.authorization = `Bearer ${tokenResponse}`; // Change as per requirement
...
export const extractData = (r: any) => r.data;
